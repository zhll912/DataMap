...
namespace DataMap.DAL
{
    ...
    public partial class article_category
    {
            ...
            using (OleDbConnection conn = new OleDbConnection(DbHelperOleDb.connectionString))
            {
                ...
                using (OleDbTransaction trans = conn.BeginTransaction())
                {
                    try
                    {
                        ...
                        //修改节点列表和深度
                        DbHelperOleDb.ExecuteSql(conn, trans, "update " + databaseprefix + "article_category set class_list='" + model.class_list + "', class_layer=" + model.class_layer + " where id=" + model.id); //带事务
                        trans.Commit();
                    }
                    catch
                    {
                ...
                using (OleDbTransaction trans = conn.BeginTransaction())
                {
                    try
                    {
                        ...
                        if (IsContainNode(model.id, model.parent_id))
                        {
                            ...
                            //先提升选中的父节点
                            DbHelperOleDb.ExecuteSql(conn, trans, "update " + databaseprefix + "article_category set parent_id=" + oldModel.parent_id + ",class_list='" + class_list + "', class_layer=" + class_layer + " where id=" + model.parent_id); //带事务
        ...
        /// 得到一个对象实体(重载，带事务)
        /// </summary>
        public Model.article_category GetModel(OleDbConnection conn, OleDbTransaction trans, int id)
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select top 1 id,channel_id,title,call_index,parent_id,class_list,class_layer,sort_id,link_url,img_url,content,seo_title,seo_keywords,seo_description");
            strSql.Append(" from " + databaseprefix + "article_category ");
        ...
        private void UpdateChilds(OleDbConnection conn, OleDbTransaction trans, int parent_id)
        {
            ...
            if (model != null)
            {
                ...
                DataSet ds = DbHelperOleDb.Query(conn, trans, strSql); //带事务
                foreach (DataRow dr in ds.Tables[0].Rows)
                {
                    ...
                    DbHelperOleDb.ExecuteSql(conn, trans, "update " + databaseprefix + "article_category set class_list='" + class_list + "', class_layer=" + class_layer + " where id=" + id); //带事务

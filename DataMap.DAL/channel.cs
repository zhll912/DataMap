...
namespace DataMap.DAL
{
	...
	public partial class channel
	{
        ...
        /// 将对象转换实体
        /// </summary>
        public Model.channel DataRowToModel(OleDbConnection conn, OleDbTransaction trans, DataRow row)
        {
            Model.channel model = new Model.channel();
            if (row != null)
            {
                ...
                OleDbParameter[] parameters2 = {
					    new OleDbParameter("@channel_id", OleDbType.Integer,4)};
                parameters2[0].Value = model.id;
                DataSet ds2 = DbHelperOleDb.Query(conn, trans, strSql2.ToString(), parameters2);
        ...
        /// 删除及重建该频道视图
        /// </summary>
        public void RehabChannelViews(OleDbConnection conn, OleDbTransaction trans, Model.channel model, string old_name)
        {
            //删除旧的视图
            StringBuilder strSql1 = new StringBuilder();
        ...
        /// 删除已移除的频道扩展字段
        /// </summary>
        private void FieldDelete(OleDbConnection conn, OleDbTransaction trans, List<Model.channel_field> models, int channel_id)
        {
            StringBuilder strSql = new StringBuilder();
            ...
            foreach (DataRow dr in ds.Tables[0].Rows)
            {
                ...
                if (model == null)
                {
                    DbHelperOleDb.ExecuteSql(conn, trans, "delete from " + databaseprefix + "channel_field where channel_id=" + channel_id + " and field_id=" + dr["field_id"].ToString()); //删除该字段
                }
            }
        }
        #endregion

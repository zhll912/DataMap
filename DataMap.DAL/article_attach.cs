...
namespace DataMap.DAL
{
    ...
    public partial class article_attach
    {
        ...
        {
            string strSql = "select top 1 id from " + databaseprefix + "article_attach order by id desc";
        ...
        /// </summary>
        public void DeleteList(OleDbConnection conn, OleDbTransaction trans, List<Model.article_attach> models, int article_id)
        {
            ...
            {
                int rows = DbHelperOleDb.ExecuteSql(conn, trans, "delete from " + databaseprefix + "article_attach where id=" + dr["id"].ToString()); //删除数据库
                if (rows > 0)
                {

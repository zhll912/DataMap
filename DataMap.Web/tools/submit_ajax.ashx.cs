...
using DataMap.Web.UI;
using DataMap.Common;

namespace DataMap.Web.tools
{
    /// <summary>
    /// submit_ajax 的摘要说明
    /// </summary>
    public class submit_ajax : IHttpHandler, IRequiresSessionState
    {
        ...
        private void user_login(HttpContext context)
        {
            ...
            {
                Utils.WriteCookie(DTKeys.COOKIE_USER_NAME_REMEMBER, "DataMap", model.user_name, 43200);
                Utils.WriteCookie(DTKeys.COOKIE_USER_PWD_REMEMBER, "DataMap", model.password, 43200);
            }
            else
            {
                //防止Session提前过期
                Utils.WriteCookie(DTKeys.COOKIE_USER_NAME_REMEMBER, "DataMap", model.user_name);
                Utils.WriteCookie(DTKeys.COOKIE_USER_PWD_REMEMBER, "DataMap", model.password);
            }
        ...
        private void user_invite_code(HttpContext context)
        {
            ...
            {
                int result = codeBll.GetCount("user_name='" + model.user_name + "' and type='" + DTEnums.CodeEnum.Register.ToString() + "' and datediff('d',add_time,date())=0");
            ...
            //删除过期的邀请码
            codeBll.Delete("type='" + DTEnums.CodeEnum.Register.ToString() + "' and status=1 or datediff('d',eff_time,date())>0");
            //随机取得邀请码
        ...
        private void order_save(HttpContext context)
        {
            ...
            foreach (Model.cart_items item in goodsList)
            {
                ...
                //添加进订单列表
                gls.Add(new Model.order_goods { article_id = item.article_id, goods_no = item.goods_no, goods_title = item.title, 
